		...
		<view class="fixed lf0 bgBlack zdx100 w100">
			<view class="flex between alcenter content plr20 w100 h50">
				<view class="content-left flex baseline jscenter bold">
					<view :class="Type=='sell'?' ft24 mr10':'gray75 ft14 mr10'" @click="changetype('sell')">{{$t('legal.ibuy')}}</view>
					<view :class="Type=='buy'?' ft24':'gray75 ft14'" @click="changetype('buy')">{{$t('legal.isell')}}</view>
				</view>
			...
			<view class="bdb27 bgPart rat4">
				<scroll-view class="tab-bar nowrap plr10 flex alcenter" scroll-x="true">
					<view class="uni-tab-item" v-for="(item,index) in legal_list" :key="index">
						<view :class="coinindex==index?'mlr10 h40 lh40 ft13 gray75 blue21 coin':'mlr10 h40 lh40 ft13 gray75'" @click="changeCoin(index,item.id)">{{item.code}}</view>
					</view>
				</scroll-view>
			</view>
		</view>
			...
			<view class="gray75 flex column w100 plr20 ptb10 bdb27" v-for="item in orderlist" :key="item.id">
				<view class="flex between mt5">
					<view class=" flex alcenter">
						<view class="logo bgBlue2 ft12 flex alcenter jscenter white" v-if="item.seller_name">{{item.seller_name || '' | chart0}}</view>
				...
				<view class="flex between mt5">
					<view class="flex alcenter">
						...
						<text class="pl5">{{item.min_number | toFixed2}}{{item.area_currency}}-{{item.max_number | toFixed2}}{{item.area_currency}}</text>
					</view>
					<view class="flex alcenter">
						<text class="blue21 ft16">{{item.price}}{{item.area_currency}}</text>
					</view>
				</view>
				<view class="flex between mt5">
					<view class="flex alcenter">
					...
					<view class="flex alcenter jscenter h25 plr20 bgBlue2 radius2" @click="getinfo(item)">
						<text class="white ft14">{{Type=='sell'?$t('legal.buy'):$t('legal.sell')}}</text>
					</view>
				</view>
			</view>
				...
				<view class="bggray flex between alcenter plr15 ptb10">
					<view class=" flex column">
						...
						<view class="mt5 ft14">
							...
							<text class="blue21 price pl5" v-if="Data.price"> {{Data.price | toFixed2}} </text> 
							<text class="blue21 price pl5">{{Data.area_currency}}</text>
						</view>
					</view>
				...
				<view class="mtb10 tab flex column plr15">
					...
					<view class="flex between alcenter Ipt ft13 pl10 mt10 bd3a" v-if="msg==0">
						...
						<view class="ptb10 flex w40 jscenter w40">
							...
							<text class="plr10 blue21 tc" @click="all(0)">{{$t('trade.all')}}{{Type=='sell'?$t('trade.sell'):$t('trade.buy')}}</text>
						</view>
					</view>
					<view class="flex between alcenter Ipt ft13 pl10 mt10 bd3a" v-if="msg==1">
						...
						<view class="ptb10 flex w40 jscenter w40">
							...
							<text class="plr10 blue21 tc" @click="all(1)">{{$t('trade.all')}}{{Type=='sell'?$t('trade.buy'):$t('trade.sell')}}</text>
						</view>
					</view>
					...
					<view class="ft14 flex alcenter between mt10">
						...
						<text class="blue21 ft16 total">{{totalPrice | toFixed2}} {{Data.area_currency}}</text>
