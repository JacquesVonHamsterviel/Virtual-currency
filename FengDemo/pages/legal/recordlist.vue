<template>
  <view class="" :class="{ light: theme == 'light' }">
    ...
    <view class="pt50">
      ...
      <view>
        <view
          class="gray75 flex column w100 plr15 ptb15 bdb27"
          v-for="(item, index) in orderlist"
          :key="index"
          @click="goDetail(item.id, item.is_sure)"
        >
          <view class="flex between">
            <view class="flex alcenter ft13 bold">
              <text :class="['blue21 mr5 ft14', { redColor: item.way == 'SELL' }]">{{
                item.way == "SELL" ? $t("legal.sell") : $t("legal.buy")
              }}</text>
          ...
          <view class="flex between mt10">
            ...
            <view class="flex column flexend">
              <text v-if="item.area_info"
                >{{ $t("legal.allmoney") }}({{ item.area_info.currency }})</text
              >
        ...
        <view class="tc gray7 ptb20" v-show="!hasMore && orderlist.length > 10">{{
          $t("home.nomore")
        }}</view>
      </view>
    </view>
      ...
      <view class="bgPart" @click.stop>
        <view class="pt10 plr10 ft10">
          ...
          <view class="select deal_type mt10 mb10 flex">
            <view :class="way == 'BUY' ? 'active' : 'baseBg'" @click="changetype('BUY')">{{
              $t("legal.buy")
            }}</view>
            <view :class="way == 'SELL' ? 'active' : 'baseBg'" @click="changetype('SELL')">{{
              $t("legal.sell")
            }}</view>
          </view>
          ...
          <view class="select deal_statu mt10 mb10 flex wraps">
            <!-- 0 交易中 1已付款 2延期确认 3维权 4取消 5确认 -->
            <view :class="status === 0 ? 'active' : 'baseBg'" @click="changestatu(0)">{{
              $t("store.notdone")
            }}</view>
            <view :class="status === 1 ? 'active' : 'baseBg'" @click="changestatu(1)">{{
              $t("trade.has_pay")
            }}</view>
            <view :class="status === 2 ? 'active' : 'baseBg'" @click="changestatu(2)"
              >延期确认</view
            >
            <view :class="status === 3 ? 'active' : 'baseBg'" @click="changestatu(3)">{{
              $t("store.protection")
            }}</view>
            <view :class="status === 4 ? 'active' : 'baseBg'" @click="changestatu(4)">{{
              $t("trade.has_cancel")
            }}</view>
            <view :class="status === 5 ? 'active' : 'baseBg'" @click="changestatu(5)">{{
              $t("store.done")
            }}</view>
          </view>
        </view>
        <view class="flex alcenter bdt2f">
          <view class="flex1 tc ptb10 reset gray75" @click="reset()">{{
            $t("store.chongzhi")
          }}</view>
          <view class="active flex1 tc ptb10 bgBlue2 white confirm" @click="confirm">{{
            $t("login.e_confrim2")
          }}</view>
        </view>
      </view>
...
export default {
  ...
  methods: {
    ...
    reset() {
      ...
      this.show = !this.show;
      this.page = 1;
      (this.hasMore = true), (this.over = false), this.getData();
    },
    confirm() {
      this.show = false;
      this.page = 1;
