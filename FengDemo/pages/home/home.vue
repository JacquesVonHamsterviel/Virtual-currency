<template>
	<view :class="{ light: theme == 'light' }" class="content " >
		...
		<!--涨幅榜-->
		<swiper display-multiple-items="3" class="ptb10 h100 w100 bgPart" v-if="quoList.length > 1">
...
export default {
	data() {
		return {
			...
			languages: {
				...
				hg: { name: '대한민국', img: '/static/image/hg.png' },
				yd: { name: 'हिन्दी', img: '/static/image/yd.png' },
			},
	...
	methods: {
		changeFooter() {
			...
			uni.setTabBarItem({
				index: 1,
				text: this.$t('trade.hall')
			});
			uni.setTabBarItem({
				index:2,
				text: this.$t('legal.list')
			});
			uni.setTabBarItem({
				index: 3,
			...
			// uni.setTabBarItem({
			// 	index: 4,
			// 	text: this.$t('assets.legal')
			// });
		},
		 onurl(){
			// plus.runtime.openUrl('https://changelly.com/buy');
			 //window.location.href = `https://changelly.com/buy`;
			 //console.log(22222)
		 },
		// 切换主题
		switchChange(e){
